...
import { SALES_DEMO_INR_PER_USD } from '@/lib/platform/admin-demo';
...
function AdminSalesPartnersPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="border-border bg-card/80 rounded-2xl border p-4 sm:p-5">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-stretch">
        <Card className="border-border bg-card flex-1 p-4 sm:p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="border-border bg-muted/30 flex items-center gap-2 rounded-lg border p-1">
              <Button
                type="button"
                size="sm"
                ...
                {data.regionPulse.map((r) => (
                  <li key={r.regionId}>
                    <div className="flex items-center justify-between gap-2 text-xs">
                      <span className="text-muted-foreground truncate font-mono">{r.label}</span>
                      <span
                        className={cn(
        ...
        <Card
          className={cn(
            'border-border bg-card flex-1 p-4 transition sm:p-5',
            stuckLens && 'ring-warning/40 ring-2',
          )}
        >
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
          ...
          <ul className="space-y-3">
            {leakageRows.map((row) => {
              ...
              return (
                <li
                  key={row.id}
                  className="border-border bg-muted/20 rounded-xl border px-3 py-3 sm:flex sm:items-center sm:justify-between sm:gap-3"
                >
                  <div className="min-w-0 space-y-1">
                    ...
                    <p className="text-muted-foreground text-[11px]">{row.meta}</p>
                  </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="border-border bg-card flex flex-col lg:min-h-[420px]">
          ...
          <div className="flex flex-1 flex-col gap-3 p-4 sm:p-5">
            {data.auditLog.map((e: SalesCommercialAuditEntry) => {
              ...
              return (
                ...
                >
                  <div className="bg-background text-foreground flex h-9 w-9 shrink-0 items-center justify-center rounded-lg shadow-sm">
                    <Icon className="h-4 w-4" />
                  </div>
                  <div className="min-w-0 flex-1 space-y-2">
                    <p className="text-foreground text-xs leading-relaxed">{e.summary}</p>
                    <p className="text-muted-foreground font-mono text-[10px]">{e.at}</p>
      ...
      <Card className="border-border bg-card flex flex-wrap items-center justify-between gap-3 p-4 sm:p-5">
            ...
            <tbody>
              {data.partnerLeaderboard.map((p) => (
                ...
                >
                  ...
                  <td className="text-muted-foreground px-5 py-3.5 font-mono text-xs">{p.code}</td>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {data.partnerLeaderboard.slice(0, 2).map((p) => (
          ...
          >
            <div className="text-muted-foreground text-xs">Target progress · {p.name}</div>
            <Progress
              value={p.targetPct}
              className="mt-3 h-2"
            />
