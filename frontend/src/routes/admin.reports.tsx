...
function ReportCard({
  ...
  return (
    <Card
      className={cn(
        'border-border bg-card hover:border-foreground/15 flex flex-col justify-between gap-4 p-5 transition hover:shadow-sm',
        report.legacy && 'ring-border/80 ring-1',
      )}
    >
        ...
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="min-w-0 space-y-1">
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="leading-snug font-medium">{report.title}</h3>
              {report.legacy && (
                <Badge
                  variant="secondary"
    ...
    <TooltipProvider delayDuration={250}>
      <div className="space-y-8">
        ...
        {(Object.keys(CATEGORY_LABEL) as ReportCategory[]).map((cat) => (
          ...
          >
            <div>
              ...
              <p className="text-muted-foreground text-xs">{CATEGORY_LABEL[cat].subtitle}</p>
            </div>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <Card className="border-border from-card to-muted/20 bg-gradient-to-br p-5 lg:col-span-2">
            <div className="text-muted-foreground flex items-center gap-2 text-[11px] font-medium tracking-[0.14em] uppercase">
              <Sparkles className="h-3.5 w-3.5" />
              Velon AI · smart narrative
            </div>
            <p className="text-foreground mt-3 text-sm leading-relaxed">{narrative}</p>
          ...
          <Card className="border-border bg-card p-5">
            ...
            <ul className="text-muted-foreground mt-3 space-y-2 text-xs">
              <li>
                <span className="text-foreground font-medium">Anomaly:</span>{' '}
              ...
              <li>
                <span className="text-foreground font-medium">Forecast:</span> MRR trajectory
                implies {formatCurrency(Math.round(o.mrrTotal * (1 + o.arrGrowthPct / 100)))} next
                period if growth holds.
              </li>
              <li>
                <span className="text-foreground font-medium">Infra:</span> Replication lag
