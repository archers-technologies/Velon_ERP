    ...
    >
      ...
      <div className="grid gap-5 md:grid-cols-3">
        {plans.map((plan) => (
          ...
          >
            {plan.featured && (
              <Badge className="bg-background text-foreground hover:bg-background mb-3">
                Most popular
              </Badge>
            )}
            <h3 className="text-lg font-semibold">{plan.name}</h3>
            <p className="mt-2 text-3xl font-semibold tracking-tight">
              {plan.isCustom ? 'Custom' : `${formatPlanPriceForPreference(plan, preference)}/mo`}
            </p>
            <p
              className={`mt-1 text-sm ${plan.featured ? 'text-background/70' : 'text-muted-foreground'}`}
            >
              {plan.desc}
            </p>
