...
function AdminOverviewPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-2xl border border-border bg-card/80 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
        <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Card className="border-border bg-card p-4 sm:p-5 lg:col-span-2">
          <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <div className="mt-1 flex flex-wrap items-baseline gap-2">
                ...
                <span className="text-sm font-medium text-success">↑ {o.arrGrowthPct}%</span>
              </div>
            </div>
          ...
          <ChartContainer
            ...
            className="aspect-[21/9] w-full min-h-[200px] max-h-[280px]"
          >
            <AreaChart data={o.revenueByMonth} margin={{ left: 0, right: 8, top: 8, bottom: 0 }}>
              <defs>
        ...
        <Card className="border-border bg-card p-4 sm:p-5">
          ...
          <div className="mt-5 rounded-xl bg-gradient-pale p-4">
            <div className="flex items-center gap-2 text-sm font-medium">
              <Globe className="h-4 w-4" /> 47 countries
            </div>
            <p className="mt-1 text-xs text-muted-foreground">Top: US · IN · DE · UK · AU</p>
          </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="border-border bg-card p-4 sm:p-5">
          ...
          <ChartContainer
            ...
            className="aspect-[16/9] w-full min-h-[180px] max-h-[240px]"
          >
            <BarChart
            ...
            >
              ...
              />
              <ChartTooltip content={<ChartTooltipContent />} />
              <Bar dataKey="newTenants" fill="var(--color-newTenants)" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ChartContainer>
        </Card>
        ...
        <Card className="border-border bg-card p-4 sm:p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Plan distribution across tenants</p>
            </div>
          </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
          ...
          <ul className="space-y-2">
            {o.activityFeed.map((item) => {
              ...
              return (
                <li
                  key={item.id}
                  className={`flex gap-3 rounded-lg border border-l-4 py-2.5 pl-3 pr-2 ${activityTone(item.severity)}`}
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 text-[10px] text-muted-foreground">{item.timeLabel}</p>
                  </div>
                </li>
              );
            })}
        ...
        <Card className="border-border bg-card p-4 sm:p-5">
          ...
          <ul className="space-y-2.5 font-mono text-[11px] leading-relaxed">
            {o.systemLogs.map((log) => (
              <li key={log.id} className="rounded-md border border-border bg-muted/20 px-2 py-1.5">
                ...
                <p className="mt-1 text-foreground/90">{log.message}</p>
              </li>
            ))}
          </ul>
          <Button variant="outline" size="sm" className="mt-4 w-full rounded-lg" asChild>
        ...
        <Card className="relative overflow-hidden border-border bg-foreground p-4 text-background sm:p-5">
          <div className="absolute -right-12 -top-12 h-44 w-44 rounded-full bg-background/10 blur-3xl" />
          <div className="relative">
            <div className="mb-3 flex items-center gap-2">
              <Sparkles className="h-4 w-4" />
              <div className="text-sm font-semibold">Agentic signals</div>
            </div>
            <p className="mb-3 text-[11px] text-background/70">
            ...
            <ul className="space-y-2 text-xs">
              {[
                ...
                <li
                  key={t}
                  className="rounded-lg border border-background/10 bg-background/10 p-2 leading-relaxed"
                >
                  {t}
                </li>
              ))}
            ...
            <thead className="bg-muted/40 text-[10px] uppercase tracking-[0.14em] text-muted-foreground">
              <tr>
                {["Business", "Plan", "Region", "Users", "MRR", "Status", ""].map((h) => (
                  <th key={h} className="px-4 py-3 text-left font-medium sm:px-5">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {tenants.map((t) => (
                <tr key={t.id} className="border-t border-border hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 text-muted-foreground sm:px-5 sm:py-3.5">{t.country}</td>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card className="border-border bg-card p-4 sm:p-5">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            ...
            <Link to="/admin/infrastructure" className="underline underline-offset-2">
              Infrastructure
            </Link>{" "}
