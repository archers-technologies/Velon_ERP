    ...
    return (
      <Card className="border-border bg-card p-8 text-center">
        <p className="text-muted-foreground text-sm">
          Tenant Owner access is required for workspace administration.
        </p>
        <Button
          asChild
          variant="outline"
                ...
                <div className="mt-2 flex flex-wrap items-center gap-3">
                  ...
                  {companyForm.logoDataUrl ? (
                    <Button
                      ...
                      variant="ghost"
                      size="sm"
                      onClick={() => setCompanyForm((prev) => ({ ...prev, logoDataUrl: '' }))}
                    >
                      Remove logo
                    </Button>
                  ) : null}
            ...
            {overview.workspace && (
              ...
              >
                ...
                <p className="text-muted-foreground font-mono text-xs">
                  URL: /app · slug: {overview.workspace.slug}
                </p>
                <div>
                  <Label>Timezone</Label>
                  <Select
                    value={workspaceForm.timezone}
                    onValueChange={(v) => setWorkspaceForm((f) => ({ ...f, timezone: v }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setWorkspaceForm((f) => ({ ...f, currency: v }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setWorkspaceForm((f) => ({ ...f, language: v }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
          ...
          <Card className="border-border bg-card p-6">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-3">
              <div>
                ...
                <p className="text-xl font-semibold">{overview.seats.plan}</p>
              </div>
              <div>
                <p className="text-muted-foreground text-xs">Active seats</p>
                <p className="text-xl font-semibold">
                  {overview.seats.activeSeats}
                  {overview.seats.unlimited ? '' : ` / ${overview.seats.limit}`}
                </p>
              </div>
              <div>
                <p className="text-muted-foreground text-xs">Remaining</p>
                <p className="text-xl font-semibold">
                  {overview.seats.unlimited ? 'Unlimited' : overview.seats.remaining}
                </p>
              </div>
            </div>
          </Card>
                ...
                {editingDeptId === d.id ? (
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      ...
                      <p className="text-muted-foreground text-xs">
                        {d.description ?? '—'} · {d.memberCount} members
                      </p>
                    </div>
                    <div className="flex gap-2">
                      <Button
                      ...
                      <Button
                        ...
                        variant="ghost"
                        onClick={() =>
                          void deleteTenantDepartment(d.id)
                            .then(refreshLists)
            ...
            >
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setInviteForm({ ...inviteForm, departmentId: v === 'none' ? '' : v })
                  }
                >
                  <SelectTrigger>
            ...
            {invitations.map((inv) => (
              ...
              >
                <div>
                  ...
                  <p className="text-muted-foreground text-xs">
                    {inv.status} · expires {new Date(inv.expiresAt).toLocaleDateString()}
                  </p>
                </div>
                {inv.status === 'PENDING' && (
                  <div className="flex gap-2">
                    <Button
                      ...
                      variant="outline"
                      onClick={() =>
                        void resendTenantInvitation(inv.id).then((r) => {
                          toast.success('Invitation resent');
                          if (r.devInviteUrl) toast.info(r.devInviteUrl);
                        })
                      }
                    >
                      Resend
                    ...
                    <Button
                      ...
                      variant="ghost"
                      onClick={() =>
                        void revokeTenantInvitation(inv.id)
                          .then(refreshLists)
        ...
        >
          <Card className="border-border bg-card divide-y">
            ...
            {overview.auditLogs.length === 0 && (
              <p className="text-muted-foreground p-6 text-sm">No audit events yet.</p>
            )}
          </Card>
        </TabsContent>
      </Tabs>
