...
function InventoryWarehousesPage() {
  ...
  useEffect(() => {
    refresh().catch((e) => toast.error(e instanceof Error ? e.message : "Failed to load warehouses"));
  }, [refresh]);
      ...
      {canManage && (
        <Card className="border-border bg-card p-4">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-3">
            ...
            <div>
              <Label>Code</Label>
              <Input value={form.code} onChange={(e) => setForm((f) => ({ ...f, code: e.target.value }))} placeholder="Auto if empty" />
            </div>
            <div>
          ...
          <Button className="mt-3" size="sm" disabled={busy || !form.name.trim()} onClick={handleSave}>
              ...
              <TableRow key={w.id}>
                ...
                {canManage && (
                  <TableCell className="text-right">
                    <Switch checked={w.isActive} disabled={busy} onCheckedChange={() => toggleActive(w)} />
                  </TableCell>
                )}
              </TableRow>
            ))}
