...
export function formatApiError(err: unknown, fallback = 'Request failed'): string {
  ...
  if (
    ...
    /Cannot (GET|POST|PATCH|DELETE) \/api/i.test(err.message)
  ) {
  ...
  if (
    import.meta.env.DEV &&
    err instanceof Error &&
    /^internal server error$/i.test(err.message.trim())
  ) {
  ...
  if (err instanceof Error && /unexpected error occurred/i.test(err.message)) {
    return import.meta.env.DEV
      ? 'API rejected the request (often CORS or API not fully started). Run npm run dev and wait for “Velon API listening on port 3001”.'
      : 'Server error while signing in. Please try again, and contact support if it continues.';
  }
