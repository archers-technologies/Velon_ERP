...
import { requestPasswordResetOtp, verifyPasswordResetOtp } from './helpers/password-reset-otp';
...
describeIfDb('Password reset OTP flow', () => {
  ...
  it('returns the same generic message for unknown and known emails', async () => {
    ...
    expect(body<{ message: string }>(known).message).toBe(
      body<{ message: string }>(unknown).message,
    );
  });
