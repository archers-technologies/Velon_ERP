...
export function smtpConfigured(): boolean {
  ...
  return (
    summary.hostPresent && summary.userPresent && summary.passwordPresent && summary.fromPresent
  );
}
...
export function parseResendSendFailure(statusCode: number, body: string): ResendSendFailureDetail {
  ...
  try {
    const json = JSON.parse(body) as { message?: string; error?: { message?: string } } | undefined;
...
export async function sendTransactionalMail(input: TransactionalMail): Promise<{
  delivered: boolean;
...
async function sendViaSmtp(input: TransactionalMail): Promise<{
  delivered: boolean;
  skippedReason?: string;
  failureDetail?: SmtpSendFailureDetail;
}> {
  ...
  try {
    ...
    const primaryFailure = message.includes('smtp_timeout')
      ? ({
