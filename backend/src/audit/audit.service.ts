...
export class AuditService {
  ...
  async logLogin(input: Omit<AuditInput, "action" | "entityType">) {
    return this.log({ ...input, action: "auth.login", entityType: "user", entityId: input.actorId });
  }

  async logLogout(input: Omit<AuditInput, "action" | "entityType">) {
    return this.log({ ...input, action: "auth.logout", entityType: "user", entityId: input.actorId });
  }

  async logRegistration(input: Omit<AuditInput, "action" | "entityType">) {
    return this.log({ ...input, action: "auth.registration", entityType: "user", entityId: input.actorId });
  }

  async logWorkspaceCreated(input: Omit<AuditInput, "action" | "entityType">) {
    return this.log({ ...input, action: "workspace.created", entityType: "workspace", entityId: input.entityId });
  }
