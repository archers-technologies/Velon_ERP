...
function canSeedDemoTenants(env = process.env) {
    ...
    if (env.NODE_ENV === 'test')
        return true;
    if (env.NODE_ENV === 'development' && env.SEED_DEMO_DATA === 'true')
        return true;
...
function inferDemoSeedSourceFromEmail(email) {
    ...
    if (normalized.endsWith('@platform.test'))
        return 'e2e';
    if (normalized.endsWith('@demo-retail.local'))
        return 'demo';
    return null;
}
function inferDemoSeedSourceFromTenantName(name) {
    const n = name.trim();
    if (/^Demo Retail Co\.?$/i.test(n))
        return 'demo';
    if (/^(Release Flow Corp|Reactivation Co|Isolation Corp|CRM Corp|Perm Matrix Corp|Billing Corp|Submit Corp|Audit Co|Admin Corp|Quote Corp|Sales Corp)\b/i.test(n)) {
        return 'e2e';
    }
    if (/\b(Corp A|Corp B)\b/.test(n) && /\d{10,}/.test(n))
        return 'e2e';
    return null;
}
