...
if (url.includes("${{") || url.includes("{{Postgres")) {
  console.error("ERROR: DATABASE_URL is unresolved — Railway could not substitute the Postgres reference.");
  console.error("");
  console.error(`Current value: ${url}`);
  console.error("");
  console.error("Fix:");
  console.error("  1. Add a PostgreSQL database to this Railway project (+ New → Database → PostgreSQL)");
  console.error("  2. On the API service, set DATABASE_URL to ${{<YourPostgresServiceName>.DATABASE_URL}}");
