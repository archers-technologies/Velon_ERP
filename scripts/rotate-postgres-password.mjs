...
function parsePgUrl(raw) {
  ...
  if (!match) {
    throw new Error(
      'DATABASE_URL must look like postgresql://user:pass@host:port/db?schema=public',
    );
  }
...
if (psql.status !== 0) {
  ...
  console.error(
    'For Railway hosted Postgres, see docs/HOSTING.md (section: Rotate database password).',
  );
...
console.log(
  'If the API runs on Railway, update DATABASE_URL in the Railway API service variables.',
);
