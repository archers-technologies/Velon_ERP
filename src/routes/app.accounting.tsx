...
function AccountingPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          .map((k) => (
            ...
            >
              <Card className="border-border bg-card p-5 hover:border-foreground/20 hover:bg-muted/20">
                ...
                <div className="mt-2 text-[10px] font-medium text-primary">Drill down →</div>
              </Card>
            </button>
          ))}
      </div>
      ...
      <Tabs value={focusTab} onValueChange={setFocusTab} className="space-y-4">
        <TabsList className="h-auto w-full flex-wrap justify-start gap-1 bg-muted/50 p-1">
          <TabsTrigger value="tower" className="rounded-md">
            Control tower
          </TabsTrigger>
        ...
        <TabsContent value="tower" className="space-y-4 outline-none">
          <div className="grid gap-6 xl:grid-cols-5">
            <Card className="border-border bg-card p-5 xl:col-span-3">
              ...
              <ChartContainer config={cashChartConfig} className="h-[260px] w-full md:h-[280px]">
                <AreaChart accessibilityLayer data={data.cashFlowTrend}>
                  ...
                  <defs>
                    ...
                    <linearGradient id="fillOutflow" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="var(--color-outflow)" stopOpacity={0.35} />
                      <stop offset="95%" stopColor="var(--color-outflow)" stopOpacity={0.05} />
                    </linearGradient>
                  </defs>
                  <Area
                    dataKey="outflow"
        ...
        <TabsContent value="capital" className="space-y-6 outline-none">
              ...
              <div className="grid gap-2">
                {data.agingAr.map((row) => (
                  ...
                  >
                    ...
                    <div
                      className="h-8 w-3 rounded-full bg-gradient-to-t from-muted to-primary/80"
                      style={{
                        opacity: Math.min(
                          1,
              ...
              <div className="mt-4 space-y-2">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {(["Pending", "Overdue"] as const).map((status) => (
                    <Card key={status} className="border-border bg-muted/20 p-3">
                      ...
                      <div className="space-y-2">
                        {data.invoices
                          ...
                          .map((inv) => (
                            ...
                            >
                              ...
                              <div className="mt-0.5 text-muted-foreground">{inv.customer}</div>
                        ...
                        {data.invoices.filter((i) => i.status === status).length === 0 ? (
                          <p className="text-[11px] text-muted-foreground">Nothing in this lane.</p>
                        ) : null}
                      </div>
                    </Card>
                  ))}
            ...
            <div>
              ...
              <div className="flex gap-2 overflow-x-auto pb-2">
                {AP_STAGE_ORDER.map((stage) => (
                  <Card
                    key={stage}
                    className="min-w-[160px] shrink-0 border-border bg-muted/15 p-3"
                  >
                    <div className="mb-2 border-b border-border pb-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                    ...
                    <div className="space-y-2">
                      {data.apBills
                        ...
                        .map((b: ApBillRecord) => (
                          ...
                          >
                            <div className="font-medium leading-snug">{b.supplierName}</div>
                            <div className="mt-1 font-semibold tabular-nums">
                              {formatCurrency(b.amount)}
                            </div>
                            <div className="mt-1 text-muted-foreground">{b.reference}</div>
                          </div>
                        ))}
                    </div>
                  </Card>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/40 hover:bg-muted/40">
                    ...
                    <TableHead className="text-[10px] uppercase tracking-wider">JE</TableHead>
                    <TableHead className="text-[10px] uppercase tracking-wider">Source</TableHead>
                    <TableHead className="text-[10px] uppercase tracking-wider">Memo</TableHead>
                ...
                <TableBody>
                  {(data.journalEntries ?? []).map((je) => (
                    <TableRow key={je.id}>
                      ...
                      <TableCell className="text-xs text-muted-foreground">{je.postedBy}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
          ...
          <Card className="border-border bg-card p-5">
            ...
            <div className="space-y-3">
              {(data.auditLog ?? []).map((row) => (
                <div
                  key={row.id}
                  className="flex flex-wrap gap-x-4 gap-y-1 rounded-lg border border-border bg-muted/20 px-3 py-2 text-xs"
                >
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/40 hover:bg-muted/40">
                    ...
                    <TableHead className="text-[10px] uppercase tracking-wider">Match</TableHead>
                ...
                <TableBody>
                  {(data.bankFeed ?? []).map((row) => (
                    <TableRow key={row.id}>
                      <TableCell className="whitespace-nowrap text-xs">{row.bookedDate}</TableCell>
